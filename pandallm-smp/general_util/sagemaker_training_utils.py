...
def _load_and_cache_examples(cfg, tokenizer: PreTrainedTokenizer, _split="train", _file: str = None):

    print('-DD- in _load_and_cache_examples, local rank = ',smp.local_rank())
    ...
    if _file is not None:
        ...
        if _split == "train":
            ...
            if cfg.ddp_eval and smp.local_rank() != -1:
                if_barrier = True
        elif _split == "test":
    ...
    # if if_barrier and smp.local_rank() == 0:
    if if_barrier and smp.local_rank not in [-1, 0]:
