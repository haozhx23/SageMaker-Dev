...
def forward_step(model, inputs: Dict[str, torch.Tensor]):
    ...
    if isinstance(outputs, tuple):
        loss = outputs[0]
        # print(f'''--DD-- loss = outputs[0] {loss}''')
    else:
        loss = outputs["loss"]
    ...
    # lossstr = loss.item()
    # print(f'''--DD-- In @smp.step loss item is:{lossstr}, outputs:{outputs}''')
    # print(f'''--DD-- In @smp.step loss item outputs:{outputs}''')
...
def main():
    ...
    #         query_key_layer_scaling=cfg.smp_params.query_key_layer_scaling and cfg.smp_params.bf16==False,
